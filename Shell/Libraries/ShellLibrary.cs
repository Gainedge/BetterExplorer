...
namespace BExplorer.Shell
{
		...
		public sealed class ShellLibrary : ShellItem, IList<ShellItem>
		{
						...
						: this()
				{
						...
						// Get the IShellItem2
						base.m_ComInterface = ((ShellItem)sourceKnownFolder).m_ComInterface;
				...
				public bool IsPinnedToNavigationPane
				{
						get
						{
								LibraryOptions flags = LibraryOptions.PinnedToNavigationPane;
				...
				public static ShellLibrary Load(string libraryName, bool isReadOnly)
				{
						...
						try
						{
								library.m_ComInterface = (IShellItem)nativeShellItem;
				...
				public static ShellLibrary Load(string libraryName, string folderPath, bool isReadOnly)
				{
						...
						IShellItem nativeShellItem = item.m_ComInterface;
						INativeShellLibrary nativeShellLibrary = (INativeShellLibrary)new ShellLibraryCoClass();
						AccessModes flags = isReadOnly ?
										AccessModes.Read :
										AccessModes.ReadWrite;
						...
						try
						{
								library.m_ComInterface = (IShellItem2)nativeShellItem;
								library.Name = libraryName;
				...
				public void Add(ShellItem item)
				{
						...
						nativeShellLibrary.AddFolder(item.m_ComInterface);
						nativeShellLibrary.Commit();
				}
				...
				public void Clear()
				{
						...
						{
								nativeShellLibrary.RemoveFolder(folder.m_ComInterface);
						}
				...
				public bool Remove(ShellItem item)
				{
						...
						try
						{
								nativeShellLibrary.RemoveFolder(item.m_ComInterface);
								nativeShellLibrary.Commit();
						}
						catch (COMException)
						{
