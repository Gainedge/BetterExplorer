...
namespace BExplorer.Shell._Plugin_Interfaces {
  ...
  public class FileSystemListItem : IListItemEx {
    ...
    public IShellItem ComInterface => this.ParentPIDL == IntPtr.Zero ? Shell32.SHCreateItemFromIDList(this.EnumPIDL, typeof(IShellItem).GUID) : Shell32.SHCreateItemWithParent(this.ParentPIDL, null, this.EnumPIDL, typeof(IShellItem).GUID);

    /// <summary>The text that represents the display name</summary>
    public string DisplayName {
      get {
        return this.GetDisplayName(SIGDN.NORMALDISPLAY);
      }
    }
    ...
    public string ParsingName {
      get {
        return this.GetDisplayName(SIGDN.DESKTOPABSOLUTEPARSING);
      }
    }
    ...
    public string FileSystemPath {
      get {
        return this.GetDisplayName(SIGDN.FILESYSPATH);
      }
    }
    ...
    private void Initialize_Helper(IntPtr folder, IntPtr lvHandle, int index) {
      ...
      //this.ComInterface = Shell32.SHCreateItemFromIDList(this.EnumPIDL, typeof(IShellItem).GUID);
    ...
    private void Initialize_Helper2(IntPtr parent, IntPtr pidl, IntPtr lvHandle, int index) {
      ...
      this.EnumPIDL = pidl;
      //this.ComInterface = Shell32.SHCreateItemWithParent(this.ParentPIDL, null, this.EnumPIDL, typeof(IShellItem).GUID);
      this.ParentHandle = lvHandle;
    ...
    public void Initialize(IntPtr lvHandle, IntPtr pidl, int index) {
      Initialize_Helper(pidl, lvHandle, index);
    }
    ...
    public void InitializeWithShellItem(ShellSearchFolder item, IntPtr lvHandle, int index) {
      Initialize_Helper(item.Pidl, lvHandle, index);
    ...
    public IEnumerable<IntPtr> GetItemsForCount(Boolean isEnumHidden) {
      ...
      HResult navRes;
      SHCONTF flags = SHCONTF.FOLDERS | SHCONTF.FASTITEMS |
          SHCONTF.NONFOLDERS | SHCONTF.ENABLE_ASYNC | SHCONTF.INIT_ON_FIRST_NEXT;
      if (isEnumHidden) {
        flags = SHCONTF.FOLDERS | SHCONTF.INCLUDEHIDDEN | SHCONTF.INCLUDESUPERHIDDEN | SHCONTF.FASTITEMS |
          SHCONTF.NONFOLDERS | SHCONTF.ENABLE_ASYNC | SHCONTF.INIT_ON_FIRST_NEXT;
      }
      IEnumIDList enumId = ShellItem.GetIEnumIDList(folder, flags, out navRes);
      this.NavigationStatus = navRes;
      uint count;
      IntPtr pidl;
    ...
    public BitmapSource ThumbnailSource(int size, ShellThumbnailFormatOption format, ShellThumbnailRetrievalOption source) {
      ...
      IntPtr hBitmap = this.GetHBitmap(size, format == ShellThumbnailFormatOption.ThumbnailOnly, source == ShellThumbnailRetrievalOption.Default, format == ShellThumbnailFormatOption.Default);
    ...
    public IListItemEx Parent {
      get {
        IShellItem item;
        var comObject = this.ComInterface;
        HResult result = comObject.GetParent(out item);
        Marshal.ReleaseComObject(comObject);
    ...
    public IShellFolder GetIShellFolder() {
      ...
      try {
        var comObject = this.ComInterface;
        comObject.BindToHandler(IntPtr.Zero, BHID.SFObject, typeof(IShellFolder).GUID, out res); //HResult result = 
    ...
    public string ToolTipText {
      get {
        ...
        try {
          IntPtr relativePidl = this.ILPidl;
    ...
    public System.IO.DriveInfo GetDriveInfo() => this.IsDrive || this.IsNetworkPath ? new DriveInfo(ParsingName) : null;

    /// <summary>Gets the item's BitmapSource</summary>
    public BitmapSource BitmapSource {
      get {
        return this.ThumbnailSource(48, ShellThumbnailFormatOption.Default, ShellThumbnailRetrievalOption.Default);
      }
    }
    ...
    public HResult ExtractAndDrawThumbnail(IntPtr hdc, uint iconSize, out WTS_CACHEFLAGS flags, User32.RECT iconBounds, out bool retrieved, bool isHidden, bool isRefresh = false) {
      ...
      try {
        retrieved = false;
        res = thumbCache.GetThumbnail(this._Item.ComInterface, iconSize, isRefresh ? (WTS_FLAGS.WTS_FORCEEXTRACTION | WTS_FLAGS.WTS_SCALETOREQUESTEDSIZE) : (WTS_FLAGS.WTS_INCACHEONLY | WTS_FLAGS.WTS_SCALETOREQUESTEDSIZE), out bmp, flags, thumbId);
    ...
    public string GetDisplayName(SIGDN type) {
      try {
        ...
        IntPtr resultPtr = comInterface.GetDisplayName(type);
