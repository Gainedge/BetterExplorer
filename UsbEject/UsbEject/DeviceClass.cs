			...
			get {
				if (_devices == null) {
					...
					while (true) {
						...
						var interfaceDetailData = Marshal.PtrToStructure<Native.SP_DEVICE_INTERFACE_DETAIL_DATA2>(buffer);

						
						string devicePath = interfaceDetailData.DevicePath;// Marshal.PtrToStringAuto(pDevicePath);
						Marshal.FreeHGlobal(buffer);
