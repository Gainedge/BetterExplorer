...
namespace Microsoft.WindowsAPICodePack.Controls.WindowsForms {
	...
	public sealed class ExplorerBrowser :
			...
			if (iFV2 != null) {
				try {
					...
					if (hr != HResult.Ok &&
						...
						throw new CommonControlException(ShellCodePack_TransferTemp.Resources.LocalizedMessages.ExplorerBrowserViewItems, hr);
					}
		...
		public IFolderView2 GetFolderView2() {
			...
			if (this.explorerBrowserControl != null) {
				...
				switch (hr) {
					...
					default:
						throw new CommonControlException(ShellCodePack_TransferTemp.Resources.LocalizedMessages.ExplorerBrowserFailedToGetView, hr);
				}
			...
			if (iFV2 != null) {
				try {
					...
					if (hr != HResult.Ok &&
						hr != HResult.ElementNotFound &&
						hr != HResult.Fail) {
						throw new CommonControlException(ShellCodePack_TransferTemp.Resources.LocalizedMessages.ExplorerBrowserItemCount, hr);
					}
				}
				finally {
					Marshal.ReleaseComObject(iFV2);
			...
			if (iFV2 != null) {
				try {
					...
					if (hr != HResult.Ok &&
						hr != HResult.ElementNotFound &&
						hr != HResult.Fail) {
						throw new CommonControlException(ShellCodePack_TransferTemp.Resources.LocalizedMessages.ExplorerBrowserSelectedItemCount, hr);
					}
				}
				finally {
					Marshal.ReleaseComObject(iFV2);
		...
		*/
		...
		public Collumns[] AvailableColumnsList(bool All) {
			try {
				IShellFolder desktop = null;
				SHGetDesktopFolder(out desktop);
				Guid shellviewGuid = Guid.Parse(ExplorerBrowserIIDGuid.IShellView);
				IntPtr view = IntPtr.Zero;

				desktop.CreateViewObject(IntPtr.Zero, ref shellviewGuid, out view);
				Guid iid = new Guid(ExplorerBrowserIIDGuid.IColumnManager);

				IColumnManager cm = (IColumnManager)Marshal.GetObjectForIUnknown(view);
				var ishellview = (IShellView)Marshal.GetObjectForIUnknown(view);
				uint HeaderColsCount = 0;
				cm.GetColumnCount(All ? CM_ENUM_FLAGS.CM_ENUM_ALL : CM_ENUM_FLAGS.CM_ENUM_VISIBLE, out HeaderColsCount);
				Collumns[] ci = new Collumns[HeaderColsCount];
				for (int i = 0; i < HeaderColsCount; i++) {
					...
					Marshal.AllocCoTaskMem(Marshal.SizeOf(cmi));
					try {
						_GetColumnbyIndex(ishellview, All, i, out pk);
						_GetColumnInfobyIndex(ishellview, All, i, out cmi);
						col.pkey = pk;
						col.Name = cmi.wszName;
						col.Width = (int)cmi.uWidth;
						ci[i] = col;
			...
			if (explorerBrowserControl == null) {
				...
				if (hr != HResult.Ok) {
					...
					if ((hr == HResult.ResourceInUse || hr == HResult.Canceled) && NavigationFailed != null) {
						...
						throw new CommonControlException(ShellCodePack_TransferTemp.Resources.LocalizedMessages.ExplorerBrowserBrowseToObjectFailed, hr);
					}
				}
			}
		}
			...
			if (explorerBrowserControl != null) {
				...
				if ((m.Msg == (int)WindowsAPI.WndMsg.WM_LBUTTONDOWN)) {
					...
					if (!IsOldSysListView) {
						...
						IsMouseClickOutsideLV = Cursor.Position.Y <= reclv.Top || Cursor.Position.Y >= reclv.Bottom;
