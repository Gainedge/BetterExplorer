...
namespace ShellControls {
  public static class Defaults {
    public static void AddDefaultColumns(this ShellView shellView, bool isFirstOnly = false,
      ...
      var collumn =
        column.ToCollumns(new PROPERTYKEY() { fmtid = Guid.Parse("B725F130-47EF-101A-A5F1-02608C9EEBAC"), pid = 10 },
          typeof(String), false, 200);
      collumn.ID = "A0";
      if (!excludeFirst) {
        ...
        shellView.Header.Columns.Add(new ListViewColumnHeader(shellView, collumn, collumn.ID == (String.IsNullOrEmpty(shellView.LastSortedColumnId) ? "A0" : shellView.LastSortedColumnId)));
      } else {
        (shellView.Header.Columns[0] as ListViewColumnHeader).SetColWidth(collumn.Width);
      }
      ...
      if (!isFirstOnly) {
        if (!shellView.Collumns.Any(c => c.ID == "A182")) {
          ...
          var columnType =
            column2.ToCollumns(
              new PROPERTYKEY() { fmtid = Guid.Parse("B725F130-47EF-101A-A5F1-02608C9EEBAC"), pid = 4 }, typeof(Type),
              false, 150);
          columnType.ID = "A182";
          shellView.Collumns.Add(columnType);
          shellView.Header.Columns.Add(new ListViewColumnHeader(shellView, columnType, columnType.ID == (String.IsNullOrEmpty(shellView.LastSortedColumnId) ? "A0" : shellView.LastSortedColumnId)));
        }
        ...
        if (!shellView.Collumns.Any(c => c.ID == "A1")) {
          ...
          var columnSize =
            column3.ToCollumns(
              new PROPERTYKEY() { fmtid = Guid.Parse("B725F130-47EF-101A-A5F1-02608C9EEBAC"), pid = 12 }, typeof(long),
              false, 90);
          columnSize.ID = "A1";
          shellView.Collumns.Add(columnSize);
          shellView.Header.Columns.Add(new ListViewColumnHeader(shellView, columnSize, columnSize.ID == (String.IsNullOrEmpty(shellView.LastSortedColumnId) ? "A0" : shellView.LastSortedColumnId)));
        }
        ...
        if (!shellView.Collumns.Any(c => c.ID == "A3")) {
          ...
          var columnDM =
            column4.ToCollumns(
              new PROPERTYKEY() { fmtid = Guid.Parse("B725F130-47EF-101A-A5F1-02608C9EEBAC"), pid = 14 },
              typeof(DateTime), false, 150);
          columnDM.ID = "A3";
          shellView.Collumns.Add(columnDM);
          shellView.Header.Columns.Add(new ListViewColumnHeader(shellView, columnDM, columnDM.ID == (String.IsNullOrEmpty(shellView.LastSortedColumnId) ? "A0" : shellView.LastSortedColumnId)));
        }
      }

      //foreach (var collumn in shellView.Collumns) {
      //  shellView.Header.Columns.Add(new ListViewColumnHeader(shellView, collumn, collumn.ID == (String.IsNullOrEmpty(shellView.LastSortedColumnId) ? "A0" : shellView.LastSortedColumnId)));
      //}
    ...
    public static void SetColumnWidth(this Collumns col, ShellView shellViewEx) {
      ...
      var colWidth = 0;
      var colIndex = shellViewEx.Collumns.IndexOf(col);
      iiListView.GetColumnWidth(colIndex, out colWidth);
      shellViewEx.Header.Columns[colIndex].Width = colWidth;
      col.Width = colWidth;
    }
