...
namespace ShellControls {
  ...
  /// </summary>
  public partial class FolderSizeWindow : UIWindow {
    ...
    private FolderSizeWindow(String dir) {
      this.InitializeComponent();
      this._Dir = dir;
      this.Loaded += this.FOnLoaded;
    }

    public static void Open(string dir, Window owner) {
      var f = new FolderSizeWindow(dir);
      f.Owner = owner;
      f.Show();
    }

    private void FOnLoaded(object sender, RoutedEventArgs e) {
      this.LoadFolderSizeItems(this._Dir);
    }
        ...
        if (!this._SkipRefresh) {
          var t = new Thread(() => {
            ...
            Dispatcher.BeginInvoke(System.Windows.Threading.DispatcherPriority.Background,
                (Action)(() => {
                  this.FInfo = new SortableObservableCollection<FolderSizeInfoClass>(FSI.ToArray());
                  this.FInfo.Sort(s => s.FSize, ListSortDirection.Descending);
                  this.DataContext = this.FInfo;
                }));
            Thread.Sleep(100);
    ...
    private void bgw_RunWorkerCompleted(object sender, RunWorkerCompletedEventArgs e) {
      ...
      txtGettingSizes.Visibility = System.Windows.Visibility.Collapsed;
      Dispatcher.BeginInvoke(System.Windows.Threading.DispatcherPriority.Background,
        (Action)(() => {
          this.FInfo = new SortableObservableCollection<FolderSizeInfoClass>(FSI.ToArray());
          this.FInfo.Sort(s => s.FSize, ListSortDirection.Descending);
