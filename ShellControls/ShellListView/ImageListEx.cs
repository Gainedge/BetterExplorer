...
namespace ShellControls.ShellListView {
  public class ImageListEx {
    ...
    public void ResizeImages(Int32 newSize) {
      ...
      var isTiles = this._ShellViewEx.View == ShellViewStyle.Tile;
      this._IImageList.SetIconSize(newSize + (isTiles ? 30 : 10), newSize + (isTiles ? 46 : 36));
    }
    ...
    public void DrawIcon(IntPtr hdc, Int32 index, IListItemEx sho, User32.RECT iconBounds, Boolean isGhosted, Boolean isHot) {
      ...
      if (this._CurrentSize != 16) {
        ...
        if (sho.ShieldedIconIndex > 0) {
          if (this._CurrentSize > 180)
            this._Jumbo.DrawIcon(hdc, sho.ShieldedIconIndex, new Point(iconBounds.Right - this._CurrentSize / 3 - 15, iconBounds.Bottom - 8 - this._CurrentSize / 3), this._CurrentSize / 3);
          else if (this._CurrentSize > 64)
            this._Extra.DrawIcon(hdc, sho.ShieldedIconIndex, new Point(iconBounds.Right - 43 - 12, iconBounds.Bottom - 50 - 8));
          else
            this._Large.DrawIcon(hdc, sho.ShieldedIconIndex, new Point(iconBounds.Right - 33 - 12, iconBounds.Bottom - 32 - 8));
        }
        if (sho.IsShared) {
          if (this._CurrentSize > 180)
            this._Jumbo.DrawIcon(hdc, this._SharedIconIndex, new Point(iconBounds.Right - 12 - this._CurrentSize / 3, iconBounds.Bottom - 8 - this._CurrentSize / 3), this._CurrentSize / 3);
          else if (this._CurrentSize > 64)
            this._Extra.DrawIcon(hdc, this._SharedIconIndex, new Point(iconBounds.Right - 40 - 12, iconBounds.Bottom - 50 - 8));
          else
            this._Large.DrawIcon(hdc, this._SharedIconIndex, new Point(iconBounds.Right - 30 - 12, iconBounds.Bottom - 32 - 8));
        }
        ...
        if ((sho.IconType & IExtractIconPWFlags.GIL_PERCLASS) == IExtractIconPWFlags.GIL_PERCLASS) {
          ...
          if (hIconExe != IntPtr.Zero) {
            ...
            Gdi32.NativeDraw(hdc, hIconExe, (iconBounds.Left + (iconBounds.Right - iconBounds.Left - this._CurrentSize) / 2) + 4, iconBounds.Top + (iconBounds.Bottom - iconBounds.Top - this._CurrentSize) / 2,
        ...
        if (sho.IsShared) {
          this._Small.DrawIcon(hdc, this._SharedIconIndex, new Point(iconBounds.Right - 20, iconBounds.Bottom - 20));
        }
      ...
      try {
        ...
        if (queue.TryDequeue(out index)) {
          if (index < 0) {
            ...
            var result = User32.SendMessage(this._ShellViewEx.LVHandle, BExplorer.Shell.Interop.MSG.LVM_ISITEMVISIBLE, index, 0) != IntPtr.Zero;
            return result;
          }
        } else {
          return false;
    ...
    private void DrawComputerTiledModeView(IListItemEx sho, Graphics g, RectangleF lblrectTiles, StringFormat fmt) {
      ...
      if (driveInfo.IsReady) {
        ...
        var subItemTextBrush = new SolidBrush(Color.Cornsilk);
    ...
    private void DrawDefaultIcons(IntPtr hdc, IListItemEx sho, User32.RECT iconBounds) {
      if (this._CurrentSize == 16) {
        this._Small.DrawIcon(hdc, sho.IsFolder ? this._FolderFallBackIndex : this._ExeFallBackIndex,
          new Point((iconBounds.Left + (iconBounds.Right - iconBounds.Left - this._CurrentSize) / 2) + 4, iconBounds.Top + (iconBounds.Bottom - iconBounds.Top - this._CurrentSize) / 2), this._CurrentSize);
      } else if (this._CurrentSize <= 48) {
        this._Extra.DrawIcon(hdc, sho.IsFolder ? this._FolderFallBackIndex : this._ExeFallBackIndex,
        ...
        this._Jumbo.DrawIcon(hdc, sho.IsFolder ? this._FolderFallBackIndex : this._ExeFallBackIndex,
          new Point(iconBounds.Left + (iconBounds.Right - iconBounds.Left - this._CurrentSize) / 2, iconBounds.Top + (iconBounds.Bottom - iconBounds.Top - this._CurrentSize) / 2), this._CurrentSize);
      }
    }
