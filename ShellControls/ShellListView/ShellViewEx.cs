...
using ToolTip = ShellControls.ToolTip;
  ...
  public partial class ShellView : F.UserControl, IShellControl {
        ...
        this.VScroll.ValueChanged += (sender, args) => {
          ...
          if (!this._PreventScrollValueEvent) {
            ...
            if (this.View == ShellViewStyle.Details && !this.IsGroupsEnabled) {
              var itemIndex = (Int32)(Math.Ceiling(args.NewValue));
              ...
              var itemIndexFrom = (Int32)(Math.Ceiling(args.OldValue));
        ...
        if (m.Msg == (Int32)WM.WM_PARENTNOTIFY && User32.LOWORD((Int32)m.WParam) == (Int32)WM.WM_MBUTTONDOWN) {
          ...
          switch (nmhdr.code) {
            ...
            case WNM.LVN_ENDSCROLL:
              //var scrollInfoEnd = this.GetScrollPosition();
              //this._PreventScrollValueEvent = true;
              //this.OnLVScroll?.Invoke(this, new ScrollEventArgs(scrollInfoEnd) { IsPositionChangedOnly = true });
              //this.LargeImageList.ResetEvent.Set();
    ...
    protected override void OnHandleCreated(EventArgs e) {
      ...
      this.LVHandle = User32.CreateWindowEx(0, "SysListView32", String.Empty,
        ...
        (User32.WindowStyles)User32.LVS_SHOWSELALWAYS | (User32.WindowStyles)User32.LVS_AUTOARRANGE | (User32.WindowStyles)User32.LVS_NOCOLUMNHEADER, 0, -24, this.ClientRectangle.Width, this.ClientRectangle.Height + 24, this.Handle, IntPtr.Zero, hInstance, IntPtr.Zero);
    ...
    public void UpdateItem(IListItemEx obj1, IListItemEx obj2) {
      ...
      if (theItem == null) {
        ...
        this.Items.Add(obj2.Extension.Equals(".library-ms") ? FileSystemListItem.InitializeWithIShellItem(this.LVHandle, BExplorer.Shell.ShellLibrary.Load(obj2.DisplayName, true).ComInterface) : obj2);
        var col = this.AllAvailableColumns.FirstOrDefault(w => w.Value.ID == this.LastSortedColumnId).Value;
        this.SetSortCollumn(true, col, this.LastSortOrder, false);
        if (this.IsGroupsEnabled) {
          this.SetGroupOrder(false);
    ...
    public void SelectItems(IListItemEx[] shellObjectArray, Boolean isEnsureVisible = false) {
      ...
      var selectionThread = new Thread(() => {
        var shellObjectArrayFiltered = shellObjectArray.Where(w => w != null);
        var lastItem = -2;
    ...
    public void SetSortCollumn(Boolean isReorder, Collumns column, SortOrder order, Boolean reverseOrder = true) {
      ...
      try {
        ...
        this.Invoke((Action)(() => { this._IIListView.SetItemCount(this.Items.Count, 0x2); }));
    ...
    private void ScrollUpdateThreadRun(Boolean onlyValue = false) {
          ...
          foreach (var collumn in folderSettings.Columns.Elements()) {
            ...
            if (theCollumnInternal != null) {
              ...
              this.BeginInvoke((Action)(() => {
                ...
                (this.Header.Columns[colIndex] as ListViewColumnHeader).SetColWidth(theCollumnInternal.Width, !this._IsFirstNavigate2);
        ...
        if (nmlvcd.clrTextBk == -1 && nmlvcd.dwItemType == 0 && this._CurrentDrawIndex == -1) {
          ...
          if (sho != null) {
            ...
            if (this.View == ShellViewStyle.Tile) {
              ...
              var numberOfSubItems = sho.cColumns == null ? 0 : sho.cColumns.Length > 2 ? isDoubleLine ? 1 : 2 : sho.cColumns.Length;
              for (var i = 0; i < numberOfSubItems; i++) {
                ...
                if (!String.IsNullOrEmpty(val)) {
                  Gdi32.SetTextColor(hdc, textColor == null ? (int)Color.Cornsilk.ToWin32Color() : (int)textColor.Value.ToWin32Color());
                  User32.DrawText(hdc, val, -1, ref labelBoundsReal, User32.TextFormatFlags.EditControl | User32.TextFormatFlags.EndEllipsis | User32.TextFormatFlags.SingleLine | User32.TextFormatFlags.VCenter | User32.TextFormatFlags.NoPrefix);
                }
              }
            }
        ...
        switch (nmlvcd.nmcd.dwDrawStage) {
          ...
          case CustomDraw.CDDS_ITEMPREPAINT:
            if (nmlvcd.clrTextBk == -1 && nmlvcd.dwItemType == 0) {
              ...
              User32.DrawText(hdc, sho.DisplayName, -1, ref labelBoundsReal, User32.TextFormatFlags.CalcRect | User32.TextFormatFlags.Center | User32.TextFormatFlags.EditControl | User32.TextFormatFlags.WordBreak | User32.TextFormatFlags.EndEllipsis | User32.TextFormatFlags.NoPrefix);
              if (labelBottom > labelBoundsReal.Bottom) {
