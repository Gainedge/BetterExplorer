...
namespace ShellControls.ShellTreeView {
  ...
  public partial class ShellTreeViewEx : UserControl {
    ...
    private void InitTreeView() {
      ...
      this.ShellTreeView.VScrollBar = this.VScrollBar;
      this.ShellTreeView.Dock = DockStyle.Left;
      this.ShellTreeView.Location = new Point(0, 0);
      this.ShellTreeView.Height = this.ClientRectangle.Height;
      this.ShellTreeView.Width = this.ClientRectangle.Width + 18;
      ...
      this.ShellTreeView.StyleChanged += (sender, args) => {
        this.BeginInvoke((Action)(() => {
          this.ShellTreeView.Width = this.ClientRectangle.Width + this.ShellTreeView.RightOffset;
        }));
      };
      this.ShellTreeView.Resize += (sender, args) => {
    ...
    */
    ...
    private void ShellTreeView_BeforeExpand(object sender, TreeViewCancelEventArgs e) {
      ...
      if (e.Action == TreeViewAction.Expand) {
        ...
        if (e.Node.Nodes.Count > 0 && (e.Node.Nodes[0].Text == this._EmptyItemString || e.Node.Nodes[0].Text == this._SearchingForFolders)) {
          ...
          var lvSho = this.ShellListView?.CurrentFolder;
          var node = e.Node;
          ...
          var thread = new Thread(() => {
            ...
            foreach (var item in sho.ParsingName != KnownFolders.Computer.ParsingName ? sho.GetContents(this.IsShowHiddenItems)
              ...
              if (item.ParsingName.EndsWith(".library-ms")) {
                var library = BExplorer.Shell.ShellLibrary.Load(Path.GetFileNameWithoutExtension(item.ParsingName), false);
            ...
            this.BeginInvoke((Action)(() => {
              ...
              this.ShellTreeView.RequestScrollBarUpdate();
            }));
          });
          thread.SetApartmentState(ApartmentState.STA);
          thread.Start();
    ...
    private void ShellTreeView_NodeMouseClick(object sender, TreeNodeMouseClickEventArgs e) {
      if (e.Button == MouseButtons.Right) {
        if (e.Node.Tag != null) {
          new ShellContextMenu.ShellContextMenu(this.ShellListView, e.Node.Tag as IListItemEx).ShowContextMenu(this, e.Location, CMF.CANRENAME);
        }
    ...
    private void ShellTreeView_DragEnter(object sender, DragEventArgs e) {
      ...
      if (e.Data.GetDataPresent("DragImageBits")) {
        BExplorer.Shell.DropTargetHelper.DropTarget.Create.DragEnter(this.Handle, (System.Runtime.InteropServices.ComTypes.IDataObject)e.Data, ref wp, (int)e.Effect);
      } else {
        this.OnDragEnter(e);
      }
    }
    ...
    private void ShellTreeView_DragOver(object sender, DragEventArgs e) {
      ...
      if (e.Data.GetDataPresent("DragImageBits")) {
        BExplorer.Shell.DropTargetHelper.DropTarget.Create.DragOver(ref wp, (int)e.Effect);
      } else {
        this.OnDragOver(e);
      }
    }

    private void ShellTreeView_DragLeave(object sender, EventArgs e) => BExplorer.Shell.DropTargetHelper.DropTarget.Create.DragLeave();
    ...
    private void ShellTreeView_DragDrop(object sender, DragEventArgs e) {
      ...
      if (e.Data.GetDataPresent("DragImageBits")) {
        BExplorer.Shell.DropTargetHelper.DropTarget.Create.Drop((System.Runtime.InteropServices.ComTypes.IDataObject)e.Data, ref wp, (int)e.Effect);
      } else {
        this.OnDragDrop(e);
      }
    }
    ...
    public ShellTreeViewEx(System.Windows.Controls.Primitives.ScrollBar scrollbarV) {
      this.VScrollBar = scrollbarV;
    ...
    public void ChangeTheme(ThemeColors theme) {
      this.BackColor = theme == ThemeColors.Dark ? Color.Green : Color.White;
