...
namespace Microsoft.WindowsAPICodePack.Shell {
	...
	public abstract class ShellContainer : ShellObject, IEnumerable<ShellObject>, IDisposable {
		...
		internal IShellFolder NativeShellFolder {
			get {
				if (nativeShellFolder == null) {
					Guid guid = new Guid(BExplorer.Shell.Interop.InterfaceGuids.IShellFolder);
		...
		public ShellObjectCollection ToShellObjectCollection() {
			ShellObjectCollection shock = new ShellObjectCollection();

			foreach (ShellObject item in this) {
				shock.Add(item);
			}
